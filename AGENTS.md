...
- **`nix eval` is permitted** for dry-run verification of config expressions. To make unstaged changes visible to `nix eval`, temporarily stage them with `git add`, run the eval, then restore with `git restore --staged`.
